...
<div <?php kemet_blog_layout_class( 'blog-post-layout-2' ); ?>>
	...
	<?php foreach($blog_post_order as $item){ ?>
		...
		switch($item){
			case 'image':
			$overlay_image = $enable_overlay != 'none' ? kemet_addons_get_thumbnail_with_overlay( '<div class="kmt-blog-featured-section post-thumb">', '</div>' ) : kemet_get_post_thumbnail( '<div class="kmt-blog-featured-section post-thumb">', '</div>' );
