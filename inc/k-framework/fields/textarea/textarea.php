    ...
    public function shortcoder() {
      ...
      if( ! empty( $this->field['shortcoder'] ) ) {
        ...
        foreach( $shortcoders as $shortcode_id ) {
          ...
          if( isset( KFW::$args['shortcoders'][$shortcode_id] ) ) {

            $setup_args   = KFW::$args['shortcoders'][$shortcode_id];
            $button_title = ( ! empty( $setup_args['button_title'] ) ) ? $setup_args['button_title'] : esc_html__( 'Add Shortcode', 'kemet-addons' );

            echo '<a href="#" class="button button-primary kfw-shortcode-button" data-modal-id="'. $shortcode_id .'">'. $button_title .'</a>';
